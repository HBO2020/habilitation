package com.lincs.microapp.web.rest;
...
import com.lincs.microapp.IntegrationTest;
import com.lincs.microapp.config.EmbeddedKafka;
import com.lincs.microapp.config.KafkaSseConsumer;
import com.lincs.microapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class HabilitationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/habilitation-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/habilitation-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/habilitation-kafka/unregister"));
                return;
            }
        }
